...
1. Every time you write a C or JS function, it should be preceded with a doc comment that describes the intent behind this function, and importantly, only explains things that cannot be trivially inferred from reading the function signature.
2. Run E2E tests with `pnpm test:e2e` and ensure that nothing is broken. Add E2E tests for any new features or user flows you implement.

## Available skills

This project has skills:

- pebble-watchface

## Project Overview

This is a Pebble smartwatch application written in C using the Pebble SDK. Consult the `docs` folder for all of the knowledge specific to this application.

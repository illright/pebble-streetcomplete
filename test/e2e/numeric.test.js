...
function makeOsmXmlWithBuildingLevelsQuest() {
  return [
    ...
    '  </way>',
    // Building without building:levels (triggers building_levels quest)
    '  <node id="4001" lat="52.373400" lon="4.892600"/>',
    '  <node id="4002" lat="52.373400" lon="4.892800"/>',
    '  <node id="4003" lat="52.373600" lon="4.892800"/>',
    '  <node id="4004" lat="52.373600" lon="4.892600"/>',
    '  <way id="6001">',
    '    <nd ref="4001"/><nd ref="4002"/><nd ref="4003"/><nd ref="4004"/><nd ref="4001"/>',
    '    <tag k="building" v="apartments"/>',
    '    <tag k="name" v="Keizersgracht 42"/>',
    '  </way>',
    '</osm>',
  ].join('\n');
}
...
test('numeric quest: arrived flow (mocked OSM)', { concurrency: 1 }, async (t) => {
  ...
  try {
    ...
    await harness.withMockedOsm(osmXml, async () => {
      await harness.withArrivalThreshold(1500, async () => {
        ...
        await t.test('numeric screen shows action bar with plus, map, minus', { concurrency: 1 }, async () => {
          const result = await harness.assertScreenshot('numeric-screen');
          assert.ok(result.match);
        });

        await t.test('plus button increments value', { concurrency: 1 }, async () => {
          await harness.click('up', { repeat: 3 });
          await harness.delay(350);
          const result = await harness.assertScreenshot('numeric-incremented');
          assert.ok(result.match);
        });

        await t.test('minus button decrements value', { concurrency: 1 }, async () => {
          await harness.click('down');
          await harness.delay(350);
          const result = await harness.assertScreenshot('numeric-decremented');
          assert.ok(result.match);
        });

        await t.test('long press select submits the answer', { concurrency: 1 }, async () => {
          harness.clearCapturedLogs();
          await harness.click('select', { durationMs: 600 });
          await harness.delay(500);
          const thanks = await harness.assertScreenshot('numeric-thanks');
...
test('numeric quest: options menu (mocked OSM)', { concurrency: 1 }, async (t) => {
  const osmXml = makeOsmXmlWithBuildingLevelsQuest();
    ...
    await harness.withMockedOsm(osmXml, async () => {
      await harness.withArrivalThreshold(1500, async () => {
        ...
        await t.test('map button opens map screen directly', { concurrency: 1 }, async () => {
          ...
          await harness.delay(350);
          const result = await harness.assertScreenshot('numeric-map');
          assert.ok(result.match);
